...
Call log:
  - waiting for locator('button[type="submit"]').filter({ hasText: 'Search' })
    ...
    at /Users/seth/setlist-score-show-3/e2e/e2e-user-flow.spec.ts:27:26
    at /Users/seth/setlist-score-show-3/e2e/e2e-user-flow.spec.ts:16:5
```
...
- img
- textbox "Search for artists, venues, or cities...": dispatch
...
```ts
   ...
   20 |       const searchInput = page.locator('input[placeholder*="Search for artists, venues, or cities"]');
...
>  27 |       await searchButton.click();
  ...
  122 |       }
  123 |       
  124 |       // Look for add song functionality
  125 |       const addSongButton = page.locator('button').filter({ hasText: /add.*song|add.*track/i }).first();
  126 |       const addButton = page.locator('button').filter({ hasText: /^add$/i }).first();
  127 |       const selectElement = page.locator('select').first();
```