...
const ArtistCard = ({ artist }: ArtistCardProps) => {
  ...
  return (
    <Card 
      className={`bg-gray-900 border-gray-800 overflow-hidden hover:border-gray-700 transition-all duration-300 group ${isMobile ? 'native-tap' : ''}`}
    >
      <Link to={`/artists/${artist.id}/${artistSlug}`} className="block h-full">
        <div className={`${isMobile ? 'h-32' : 'h-40'} bg-gray-800 relative`}>
          ...
          {/* Source badge */}
          <div className="absolute top-2 right-2 px-2 py-1 text-xs font-medium rounded-full bg-black/70 text-gray-300 border border-gray-700">
