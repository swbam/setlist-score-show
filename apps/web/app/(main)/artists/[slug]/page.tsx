...
export default async function ArtistPage({ params }: { params: { slug: string } }) {
  ...
  try {
    const result: any = await graphqlClient.request(GET_ARTIST, { slug: params.slug })
    artistData = result.artistBySlug
  } catch (error) {
  ...
  const now = new Date()
  const upcomingShows = artistData.shows?.filter((show: any) => 
    new Date(show.date) > now
  ) || []
  
  const pastShows = artistData.shows?.filter((show: any) => 
    new Date(show.date) <= now
  ).slice(0, 10) || []
        ...
        <div className="relative container mx-auto px-4 sm:px-6 lg:px-8 py-16 sm:py-20 lg:py-24">
          <div className="flex flex-col lg:flex-row items-center gap-8 lg:gap-12">
            ...
            <div className="flex-1 text-center lg:text-left">
              <h1 className="text-4xl sm:text-5xl lg:text-6xl font-headline font-bold mb-4 bg-gradient-to-r from-white to-teal-100 bg-clip-text text-transparent">
                {artistData.name}
              </h1>
